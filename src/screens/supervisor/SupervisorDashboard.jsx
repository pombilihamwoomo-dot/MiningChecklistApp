  ...
  useEffect(() => {
    const fetch = async () => {
      ...
      const results = await Promise.all(snap.docs.map(async d => {
        ...
        return { id: d.id, ...data, workerName: userSnap.exists() ? userSnap.data().name : 'Unknown' };
      }));
    ...
    return (
      <View style={[styles.card, { borderLeftColor: passed ? '#27ae60' : '#e74c3c', borderLeftWidth: 3 }]}>
        ...
        <View style={styles.itemsRow}>
          {[
            { key: 'helmet', label: 'Helmet', icon: 'hardware-chip-outline', val: item.helmet },
            { key: 'mask', label: 'Mask', icon: 'medkit-outline', val: item.mask },
            { key: 'kit', label: 'Kit', icon: 'briefcase-outline', val: item.emergencyKit },
          ].map(i => (
